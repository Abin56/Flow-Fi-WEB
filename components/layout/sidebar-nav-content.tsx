...
import { NAV_ITEMS, type NavItem, type NavSection } from "@/components/layout/nav-items";
...
const SECTION_ORDER: NavSection[] = ["Overview", "Finance", "Planning", "Import", "Insights", "System"];

function groupBySection(items: NavItem[]): Array<{ section: NavSection; items: NavItem[] }> {
...
const SHORTCUTS = [
  { label: "Add Transaction", icon: Plus, className: "bg-primary/12 text-primary-accent-text" },
      ...
      <nav className="mt-6 flex flex-col gap-4 px-3">
        {groupBySection(NAV_ITEMS).map((group) => (
          <div key={group.section} className="flex flex-col gap-1">
            {!collapsed && (
              <p className="px-3 pb-1 text-[11px] font-medium tracking-wide text-tertiary-foreground uppercase">
                {group.section}
              </p>
            )}
            {group.items.map((item) => {
              ...
              const link = (
                <Link
                  ...
                  className={cn(
                    "flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-muted-foreground transition-colors duration-150 hover:bg-accent hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none",
          ...
          <div className="mt-2 flex flex-col gap-1">
            {SHORTCUTS.map((shortcut) => (
              <button
                ...
                type="button"
                onClick={onNavigate}
                className="flex items-center gap-3 rounded-xl px-3 py-2 text-left text-sm font-medium text-muted-foreground transition-colors hover:bg-muted/70 hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
              >
