...
12. ~~**`billsPaidThisMonth` in cash-flow figures stays 0**~~ — fixed: `features/bills/hooks/use-bill-occurrence-history.ts` now fetches every bill's full occurrence history, wired into `useCashFlowThisMonth` via `lib/engines/dashboard-aggregation.ts`'s already-ported `billsPaid`. `moneyReceivedThisMonth` is also now real (`lib/engines/cash-flow.ts`'s `moneyReceivedForRange`, a direct port of `moneyReceivedForRangeProvider` — split-expense settlement collections, not a `receiptPurpose` transaction sum, which was this file's previous incorrect assumption about what that figure represents).
13. **Bill payments previously posted a phantom `Transaction` and mutated the linked account's balance** (`postBillPaymentTransaction` in `use-bills-data.ts`) — this was a real cross-platform data-integrity bug, not a documented gap: Flutter's `BillOccurrenceRepository`/`PaymentRepository` never touch `Account`/`Transaction` when a bill is paid, so the same action produced different backend state depending on which client performed it. Fixed — bill payments now only ever update `BillOccurrence.amountPaid` and write a `PaymentRecord`, matching Flutter exactly.
