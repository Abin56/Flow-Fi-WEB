...
import { emiPrincipalRestored, emiPurchaseRepresentedOnCard, type UtilizationEmi } from "@/lib/engines/credit-utilization";
...
export function useCardUtilizationEmis(): { utilizationEmis: UtilizationEmi[]; isLoading: boolean } {
  ...
  const utilizationEmis = useMemo(() => {
    ...
    return (emis as Emi[]).map((emi): UtilizationEmi => {
  ...
  }, [emis, cards, transactions, breakdowns, installments, payments]);

  return {
    utilizationEmis,
    isLoading: emisLoading || cardsLoading || breakdownsLoading || installmentsLoading || transactionsLoading || (paidLinkedInstallments.length > 0 && paymentsQuery.isLoading),
  };
}
